...
function M.new(adapter,options)
    ...
    local function retain(s,candidate,idle)
        ...
        for key,value in pairs(candidate.entries)do value.texture=texture;self.entries[key]=value end
    ...
    function self:tick(pressure)
        ...
        for _,item in ipairs(s.items)do
            ...
            if not self.entries[item.key]then missing=missing+1 end
        end end
        self.ready_items=#ready;self.missing_ready_items=missing
